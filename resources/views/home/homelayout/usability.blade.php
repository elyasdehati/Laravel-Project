...
<div class="lonyo-section-padding bg-heading position-relative sectionn">
    <div class="container">
      ...
      <div class="row">
        ...
        @php
          $connect = App\Models\Connect::WhereIn('id',[1,2,3])->get()->keyBy('id');
        @endphp
        ...
        @foreach ($connect as $item)
          ...
          <div class="lonyo-process-wrap" data-aos="fade-up" data-aos-duration="500">
            ...
            <div class="lonyo-process-title">
              <h4 class="editable-title" contenteditable="{{ auth()->check() ? 'true' : 'false' }}" data-id="{{ $item->id }}">{{ $item->title }}</h4>
            </div>
            <div class="lonyo-process-data">
              <p class="editable-description" contenteditable="{{ auth()->check() ? 'true' : 'false' }}" data-id="{{ $item->id }}">{{ $item->description }}</p>
            </div>
          </div>
        </div>
        @endforeach
